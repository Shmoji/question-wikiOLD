...
class RichTextEditor extends React.Component {
  ...
  handleSubmit(event) {
    ...
    const { id } = this.props.match.params; // Get :id from URL (defined in Route in App.js) -- Needed to fetch this question's answers
    // TODO: create data you want to store for this answer
    client.get(`/api/question/${id}/personal`)
      .then(response => { 
        const numAnswers = response.data.data.length;
        // Store in DATABASE using axios to access backend
        const answerData = {
          user_id: this.props.auth.user.id,
          body: this.state.editorState.getCurrentContent().getPlainText(), // Value passed into text editor
          home_order: (numAnswers === 0) ? 0 : response.data.data[numAnswers - 1].home_order + 1, // 0 if first answer, else 1 greater than latest answer
        ...
        client
          .post(`/api/answer/personal`, answerData)
          .then(res => {
    ...
    // TODO: use axios to actually insert data. On then() -> reset tempState to empty. use history.push to travel to new page showing ur answer
...
export default connect(mapStateToProps)(withRouter(RichTextEditor));